...
export function TicketList({ onSelectTicket, selectedId }: TicketListProps) {
  ...
  const getStatusBadge = (status: TicketStatus) => {
    switch (status) {
      case 'open': return 'bg-blue-50 text-blue-800 border border-blue-200';
      case 'in_progress': return 'bg-orange-50 text-orange-800 border border-orange-200';
      case 'closed': return 'bg-green-50 text-green-800 border border-green-200';
    }
  };
      ...
      <div className="p-6 border-b border-slate-100 bg-[#f1f5f9]/30">
        <div className="relative">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-white border border-[#e2e8f0] rounded-sm text-sm focus:ring-1 focus:ring-blue-500 transition-all outline-none"
          />
        </div>
      </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              ...
              <tr 
                ...
                className={`group cursor-pointer hover:bg-slate-50 transition-colors ${selectedId === ticket.id ? 'bg-blue-50/50' : ''}`}
              >
                <td className="px-6 py-4">
                  <div className="flex flex-col">
                    <span className="font-mono font-bold text-xs text-blue-600">#SG-{ticket.id.slice(0, 6).toUpperCase()}</span>
                    <span className="text-sm font-bold text-slate-800 line-clamp-1 mt-1 group-hover:text-blue-700">{ticket.title}</span>
                  </div>
                </td>
                <td className="px-6 py-4">
